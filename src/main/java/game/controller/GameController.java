...
public class GameController {
    ...
    private void handleClickIfIsHighlighted(MouseEvent mouseEvent){
        ...
        for(Moves moves:possibleMoves){
            if(row == clickedRow+moves.getRowChange() && column == clickedColumn+moves.getColChange()){
                move(moves);
                break;
            }
            eraseHighlight();
    ...
    private void move(Moves moves){
        gameState.moveTo(clickedRow, clickedColumn, moves);
        displayGameState();
        setTurnLabel();
        isHighlighted = false;
        if(this.gameState.isEnd()){
    ...
    private void highlightLegalMoves(MouseEvent mouseEvent){
        ...
        if(!piece.equals(Piece.EMPTY) && piece.equals(gameState.getTurn())) {
            ...
            for (Moves moves : possibleMoves) {
                ImageView view = (ImageView) gameGrid.getChildren().get((clickedRow + moves.getRowChange()) * 4 + clickedColumn + moves.getColChange());
    ...
    public void setGameState(int[][] state){
        this.gameState = new Statement(state);
    }
